#include "LevelBoss3p.hpp" // level header file
#include "Puzzle.hpp"
using namespace std;
using namespace sf;

void LevelBoss3p::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		...
		while (window.isOpen())
		{
			...
			custext1.Show(window, "Caution: MX8IG93 approaching", 80, 200, 200, 2, true, 0.1);

			if (custext1.hidingFinished)
			{
				enemy.move(340);
			}
			enemy.fireBullet(yuri, 1700, 1000, 505);
			enemy.fireMissile(yuri, 3000, 2500, 475);
