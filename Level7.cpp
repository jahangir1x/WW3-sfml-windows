#include "Level7.hpp" // level header file
using namespace std;
using namespace sf;

void Level7::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		cout << "in level 106 " << endl;
		Success success;
		...
		while (window.isOpen())
		{
			...
			for (auto& this_enemy : first_enemies1)
			{
				if (this_enemy.isDead == false)
				{
					...
					this_enemy.move(349);
					this_enemy.fireBullet(yuri, 6300, 5800, 670);
					this_enemy.fireMissile(yuri, 7000, 6000, 630);
