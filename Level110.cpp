...
void Level110::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		...
		vector<pair<Enemy1, Enemy5>> first_enemies(2); // create 2 enemies
		vector<Enemy4> second_enemies(2);			   // create 3 enemies
		vector<pair<Enemy1, Enemy5>> third_enemies(2);

		for (auto& enemy : first_enemies)
		{
			enemy.first.setStyle(Enemy4::Style::YellowZen);


			enemy.second.setStyle(Enemy4::Style::BlueBolt);
		...
		{
			enemy.first.setStyle(Enemy4::Style::PurplePunk);


			enemy.second.setStyle(Enemy4::Style::RedRider);
		...
		while (window.isOpen())
		{
			...
			for (auto& this_enemy : first_enemies) // range based for loop + reference operator ফাস্ট হবে ।
			{
				if (this_enemy.first.isDead == false || this_enemy.second.isDead == false)
				{
					...
					this_enemy.first.fireBullet(yuri, 1000, 800, 400);
			...
			if (someone_is_alive == false)
			{
				for (i = 0; i < second_enemies.size(); i++)
				{
					if (second_enemies[i].isDead == false)
					{
						...
						second_enemies[i].fireBullet(yuri, 1200, 500, 400);
						second_enemies[i].fireMissile(yuri, 1000, 200, 295);
			...
			if (someone_is_alive == false)
			{
				for (auto& this_enemy : third_enemies) // range based for loop + reference operator ফাস্ট হবে ।
				{
					if (this_enemy.first.isDead == false || this_enemy.second.isDead == false)
					{
						...
						this_enemy.first.Show(window);
						this_enemy.second.move(340);
						this_enemy.second.fireBullet(yuri, 1500, 1000, 400);
