#include "Level107.hpp" // level header file
using namespace std;
using namespace sf;

void Level107::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		...
		vector<Enemy1> second_enemies(5); // create 3 enemies
		vector<Enemy2> third_enemies(2);
		...
		while (window.isOpen())
		{
			...
			for (auto& this_enemy : first_enemies) // range based for loop + reference operator ফাস্ট হবে ।
			{
				if (this_enemy.isDead == false)
				{
					...
					this_enemy.move(300);
					this_enemy.fireBullet(yuri, 1000, 1200, 450);
					this_enemy.fireMissile(yuri, 300, 200, 450);
			...
			if (someone_is_alive == false)
			{
				for (i = 0; i < third_enemies.size(); i++)
				{
					if (third_enemies[i].isDead == false)
					{
						...
						third_enemies[i].fireBullet(yuri, 1000, 1200, 450);
						third_enemies[i].fireMissile(yuri, 300, 200, 450);
