#include "Level6.hpp" // level header file
using namespace std;
using namespace sf;

void Level6::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		...
		vector<Enemy1> first_enemies2(3);
		vector<Enemy6> second_enemies1(3);
		vector<Enemy5> second_enemies2(3);
		vector<Enemy1> third_enemies1(3);
		vector<Enemy5> third_enemies2(3);
		...
		while (window.isOpen())
		{
			...
			for (auto& this_enemy : first_enemies1)
			{
				if (this_enemy.isDead == false)
				{
					...
					this_enemy.move(342);
					this_enemy.fireBullet(yuri, 6300, 5800, 650);
					this_enemy.fireMissile(yuri, 7000, 6000, 610);
