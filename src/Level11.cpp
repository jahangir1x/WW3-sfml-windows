#include "Level11.hpp" // level header file
using namespace std;
using namespace sf;

void Level11::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		...
		vector<Enemy2> first_enemies3(2);
		vector<Enemy3> second_enemies(6);
		vector<Enemy6> third_enemies1(2);
		vector<Enemy2> third_enemies2(4);
		vector<Enemy3> third_enemies3(1);
		vector<Enemy6> fourth_enemies1(2);
		vector<Enemy4> fourth_enemies2(2);
		vector<Enemy2> fourth_enemies3(2);
		vector<Enemy3> fourth_enemies4(2);
		...
		for (auto& enemy : fourth_enemies4)
		{
			enemy.setStyle(Enemy1::Style(3));
		}
		while (window.isOpen())
		{
			...
			for (auto& this_enemy : first_enemies1)
			{
				if (this_enemy.isDead == false)
				{
					...
					this_enemy.move(374);
					this_enemy.fireBullet(yuri, 6300, 5800, 750);
					this_enemy.fireMissile(yuri, 7000, 6000, 710);
			...
			if (someone_is_alive == false)
			{
				...
				for (auto& this_enemy : fourth_enemies4)
				{
					if (this_enemy.isDead == false)
					{
