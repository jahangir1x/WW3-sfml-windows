#include "Level13.hpp" // level header file
using namespace std;
using namespace sf;

void Level13::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		...
		vector<Enemy4> first_enemies1(3);
		vector<Enemy5> first_enemies2(2);
		vector<Enemy3> first_enemies3(2);
		vector<Enemy6> second_enemies1(3);
		vector<Enemy1> second_enemies2(3);
		vector<Enemy5> third_enemies1(5);
		vector<Enemy3> third_enemies2(1);
		vector<Enemy4> third_enemies3(1);
		vector<Enemy3> fourth_enemies1(4);
		vector<Enemy4> fourth_enemies2(4);
		vector<Enemy6> fifth_enemies1(2);
		vector<Enemy4> fifth_enemies2(2);
		vector<Enemy5> fifth_enemies3(2);
		...
		while (window.isOpen())
		{
			...
			for (auto& this_enemy : first_enemies1)
			{
				if (this_enemy.isDead == false)
				{
					...
					this_enemy.move(393);
					this_enemy.fireBullet(yuri, 6300, 5800, 770);
					this_enemy.fireMissile(yuri, 7000, 6000, 750);
