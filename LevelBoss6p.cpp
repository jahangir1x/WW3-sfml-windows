#include "LevelBoss6p.hpp" // level header file
#include "Puzzle.hpp"
using namespace std;
using namespace sf;

void LevelBoss6p::Show(RenderWindow& window, Event& event)
{
	while (LevelHelper::shouldKeepPlaying())
	{
		...
		while (window.isOpen())
		{
			...
			custext1.Show(window, "Caution: MIIKB20V approaching", 80, 200, 200, 2, true, 0.1);

			if (custext1.hidingFinished)
			{
				enemy.move(400);
			}
			enemy.fireBullet(yuri, 4000, 3500, 770);
			enemy.fireMissile(yuri, 4000, 3800, 760);
